# Terran Missions

The Terran mission track focuses on commercial-driven economic growth, exploration, and military buildup. Unlike Aspha and Guardian, Terran missions grant **no race-unique ships** — rewards are loyalty, free colonies, and infrastructure tech.

See the [Missions overview](index.md) for general mechanics. See [Terran on the Races page](../races.md#terran) for race mechanics.

## Act 1 — Independence & Self-Sufficiency

> *"These are troubled times. Your first goal is to gain independence and to become self-sufficient in all aspects of economic, military and technology superiority."*

| # | Mission | Goal | Reward |
|---|---|---|---|
| 1 | A Self Sustainable Empire | Income +500 credits/turn | +20 loyalty all colonies |
| 2 | Commercial Upgrade | Level 5 Commercial Infrastructure Tech | +20 loyalty |
| 3 | Exploration | Control 4 planets | +20 loyalty |
| 4 | Coming Out of Protection | 10,000 PR | +20 loyalty |
| 5 | Government Control Center | 5,000,000 credit reserves | **−5,000,000 credits.** +2 levels Commercial Infrastructure Tech |
| 6 | More Planets | Control 15 planets | 2 new colonies |
| 7 | Research | 1× level-20 infrastructure tech | 3 new colonies |
| 8 | Increasing Your Defenses | 50,000 PR | 1 new colony |
| 9 | Empire Influence | 2× Level 2 Cluster Colony | Advance to Act 2 |

## Act 2 — Expanding Empire Influence

| # | Mission | Goal | Reward |
|---|---|---|---|
| 1 | Financial Crisis | 50,000,000 credit reserves | **−50,000,000 credits.** +2 levels Housing Tech, +2 levels Commercial Tech |
| 2 | Resources | Control more than 50 planets | +50 loyalty |
| 3 | Military Build-Up | More than 200,000 PR | +20 loyalty |
| 4 | Military Build-Up | More than 300,000 PR | +100 loyalty |
| 5 | Alien Artifact Finding | Control 2× oceanic-class planets | +150 loyalty *(intended artifact tech reward is disabled in current game data)* |
| 6 | Expansion of the Empire | Control 80 planets | 2 new colonies |
| 7 | Stronghold | More than 800,000 PR | 3 new colonies |
| 8 | Top of the Galaxy | Control 200 planets | +250 loyalty |
| 9 | Final Mission | Control N planets (variable target) | **Infrastructure research cost cut to 1/10** (`resstart` and `resreq` divided by 10) |

## Notes

- The two "building" missions (Act 1 m_5 and Act 2 m_1) **deduct credits** equal to the reserve threshold. Don't sit exactly at the goal — leave a buffer for ongoing operations.
- Mission 9 in Act 2 awards the same global research-cost-reduction perk shared by all three race tracks, making endgame infrastructure research effectively trivial.
- Act 2 missions 5–9 share content with the Aspha Miner track.
